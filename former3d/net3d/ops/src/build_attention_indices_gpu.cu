...
__global__ void subm_strided_attention_with_hash_sort_kernel(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int num_range, int hash_size,
    ...
    */
    ...
    int num_samples = 0;
    for (int i = 0; i < search1_num * 3; i = i + 3) {
        int x_offset = search1[i];
        int y_offset = search1[i+1];
        int z_offset = search1[i+2];
        // each loop process 8 points
        if (z_offset == 0) {
            z_step = 1;
        } else {
    ...
    for (int i = 0; i < search2_num * 3; i = i + 3) {
        int x_offset = search2[i];
        int y_offset = search2[i+1];
        int z_offset = search2[i+2];
        // each loop process 8 points
        if (z_offset == 0) {
            z_step = 1;
        } else {
    ...
    for (int i = 0; i < search3_num * 3; i = i + 3) {
        int x_offset = search3[i];
        int y_offset = search3[i+1];
        int z_offset = search3[i+2];
        // printf("== search3 xyz %d: %d, %d, %d", i/3, x_offset, y_offset, z_offset);
        // each loop process 8 points
        if (z_offset == 0) {
            z_step = 1;
        } else {
    ...
    for (int i = 0; i < search4_num * 3; i = i + 3) {
        int x_offset = search4[i];
        int y_offset = search4[i+1];
        int z_offset = search4[i+2];
        // each loop process 8 points
        if (z_offset == 0) {
            z_step = 1;
        } else {
...
void subm_strided_attention_with_hash_sort_kernel_launcher(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int num_range, int hash_size,
    ...
    subm_strided_attention_with_hash_sort_kernel<<<blocks, threads>>>(x_max, y_max, z_max, num_voxels, attend_size, num_range, hash_size,
