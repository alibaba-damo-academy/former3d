...
class BuildTensorTable(Function):
    ...
    def forward(ctx, batch_size, spatial_shape, voxel_indices, v_bs_cnt):
        ...
        ops_cuda.build_mapping_with_tensor_wrapper(x_max, y_max, z_max, num_voxels, voxel_indices, v_bs_cnt, dense_map)
        return dense_map
...
class BuildHashTable(Function):
    ...
    def forward(ctx, batch_size, hash_size, spatial_shape, voxel_indices, v_bs_cnt):
        ...
        dense_map = dense_map.to(voxel_indices.device)
        ops_cuda.build_mapping_with_hash_wrapper(x_max, y_max, z_max, num_voxels, hash_size, voxel_indices, v_bs_cnt, dense_map)
        return dense_map
...
class TensorDownSample(Function):
    ...
    def forward(ctx, strides, num_ds_voxels, batch_size, spatial_shape, voxel_indices):
        ...
        ops_cuda.downsample_with_tensor_wrapper(x_max, y_max, z_max, x_stride, y_stride, z_stride,
...
class HashTableDownSample(Function):
    ...
    def forward(ctx, strides, num_ds_voxels, batch_size, hash_size, spatial_shape, voxel_indices):
        ...
        ds_voxel_indices = torch.zeros((batch_size, num_ds_voxels, 3)).int().fill_(-1).to(voxel_indices.device)
        vcount = torch.zeros(batch_size).int().to(voxel_indices.device)
        ops_cuda.downsample_with_hash_wrapper(x_max, y_max, z_max, x_stride, y_stride, z_stride,
...
class SparseLocalAttentionTensorIndices(Function):
    ...
    def forward(ctx, attend_size, attend_range, strides, dense_map, voxel_indices):
        ...
        ops_cuda.sparse_local_attention_with_tensor_wrapper(x_max, y_max, z_max, x_stride, y_stride, z_stride,
...
class SparseLocalAttentionHashIndices(Function):
    ...
    def forward(ctx, spatial_shape, attend_size, attend_range, strides, dense_map, voxel_indices):
        ...
        ops_cuda.sparse_local_attention_with_hash_wrapper(x_max, y_max, z_max, x_stride, y_stride, z_stride,
...
class SparseStridedAttentionTensorIndices(Function):
    ...
    def forward(ctx, attend_size, range_spec, strides, dense_map, voxel_indices):
        ...
        ops_cuda.sparse_strided_attention_with_tensor_wrapper(x_max, y_max, z_max, x_stride, y_stride, z_stride,
...
class SparseStridedAttentionHashIndices(Function):
    ...
    def forward(ctx, spatial_shape, attend_size, range_spec, strides, dense_map, voxel_indices):
        ...
        ops_cuda.sparse_strided_attention_with_hash_wrapper(x_max, y_max, z_max, x_stride, y_stride, z_stride,
...
class SubMLocalAttentionTensorIndices(Function):
    ...
    def forward(ctx, attend_size, attend_range, dense_map, voxel_indices):
        ...
        ops_cuda.subm_local_attention_with_tensor_wrapper(x_max, y_max, z_max,
                                                        num_voxels, attend_size, attend_range,
                                                        attend_indices, voxel_indices, dense_map)
        return attend_indices
...
class SubMLocalAttentionHashIndices(Function):
    ...
    def forward(ctx, spatial_shape, attend_size, attend_range, dense_map, voxel_indices):
        ...
        ops_cuda.subm_local_attention_with_hash_wrapper(x_max, y_max, z_max,
                                                        num_voxels, attend_size, attend_range, hash_size,
                                                        attend_indices, voxel_indices, dense_map)
        return attend_indices
...
class SubMStridedAttentionTensorIndices(Function):
    ...
    def forward(ctx, attend_size, range_spec, dense_map, voxel_indices):
        ...
        ops_cuda.subm_strided_attention_with_tensor_wrapper(x_max, y_max, z_max,
                                                            num_voxels, attend_size, num_range,
                                                            attend_indices, voxel_indices,
                                                            dense_map, range_spec)
        return attend_indices
...
class SubMStridedAttentionHashIndices(Function):
    ...
    def forward(ctx, spatial_shape, attend_size, range_spec, dense_map, voxel_indices):
        ...
        ops_cuda.subm_strided_attention_with_hash_wrapper(x_max, y_max, z_max,
                                                            num_voxels, attend_size, num_range, hash_size,
                                                            attend_indices, voxel_indices,
                                                            dense_map, range_spec)
        return attend_indices
...
class SubMStridedAttentionHashIndicesSort(Function):
    ...
    def forward(ctx, spatial_shape, attend_size, range_spec, dense_map, voxel_indices):
        ...
        with torch.no_grad():
            ...
            search_nums = []
            for r in range_spec:
                search_indices = np.stack(np.meshgrid(range(r[0], r[1], r[2]), range(r[3], r[4], r[5]), 
                                            range(r[6], r[7], r[8]))).reshape(3,-1).T.astype(np.int32)
                sorted_indices = torch.from_numpy(np.stack(sorted(search_indices, key=lambda k: k[0]**2+k[1]**2+k[2]**2))).to(range_spec.device)
                # search_indices = torch.stack(torch.meshgrid(
                #                         torch.arange(r[0], r[1], r[2]), 
                #                         torch.arange(r[3], r[4], r[5]),
                #                         torch.arange(r[6], r[7], r[8])
                #                         )).int().flatten(1).T
                # sorted_indices = torch.stack(sorted(search_indices, key=lambda k: k[0]+k[1]+k[2])).to(range_spec.device)
                search_inds.append(sorted_indices)
                search_nums.append(sorted_indices.shape[0])
            for i in range(4 - len(search_nums)):
                search_nums.append(-1)
                search_inds.append(search_inds[0][0, None])
            # search_inds = torch.stack(search_inds)
            

        ops_cuda.subm_strided_attention_with_hash_sort_wrapper(x_max, y_max, z_max,
                                                            num_voxels, attend_size, num_range, hash_size,
                                                            search_nums[0], search_nums[1], search_nums[2], search_nums[3],
                                                            attend_indices, voxel_indices,
                                                            dense_map, search_inds[0], search_inds[1], search_inds[2], search_inds[3])
        return attend_indices
...
subm_strided_attention_hash_indices_sort = SubMStridedAttentionHashIndicesSort.apply
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, features_batch_cnt: torch.Tensor,
        ...
        ops_cuda.group_features_wrapper(B, M, C, nsample, features, features_batch_cnt, idx, idx_batch_cnt, output)
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        ops_cuda.group_features_grad_wrapper(B, M, C, N, nsample, grad_out_data, idx,
